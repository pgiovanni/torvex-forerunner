...
def model_display_name(model_id: str) -> str:
    """Human name for a configured model id, for the 'what are you running on'
    answer. Built from the live .env value so a model swap can never leave the
    bot claiming the old one. Only Claude ids get the Anthropic attribution;
    anything else is reported as its raw id rather than a guessed brand."""
    name = (model_id or "").split("/")[-1].strip()
    m = re.fullmatch(r"claude-([a-z]+)-(\d+)(?:[.-](\d+))?(?:-\d{8})?", name)
    if not m:
        return name or "an unknown model"
    ver = m.group(2) + (f".{m.group(3)}" if m.group(3) else "")
    return f"Anthropic's Claude {m.group(1).capitalize()} {ver}"


def about_block(smart: str, quick: str) -> str:
    """The self-identity lines that ride at the top of the command reference.
    Without them the model has no fact about itself and, in character, guesses
    the most common chatbot name — it was telling members it ran on GPT."""
    a, b = model_display_name(smart), model_display_name(quick)
    engine = a if a == b else f"{a} (quick answers: {b})"
    return (
        "ABOUT YOU: whatever character you are playing, you are Torvex "
        f"Forerunner underneath, and your answers are generated by {engine}. If asked what AI or model you run on, say that plainly "
        "and in character. Never claim to be GPT, ChatGPT, OpenAI, Gemini, or "
        "any other model, and never suggest the model's maker endorses or "
        "runs this bot. Don't bring it up unless asked."
    )


# Questions about the bot's own engine. They load the command reference (which
# carries about_block) up front, so the answer never comes from a guess.
_MODEL_WORDS = ("gpt", "openai", "claude", "anthropic", "llm", "gemini",
                "what model", "which model", "what ai", "which ai",
                "language model", "powered by", "running on", "run on")
...
COMMAND_RULES_ONDEMAND = (
    ...
    "or whether this bot has a command for something) — or knowing what AI "
    "or model you run on — reply with exactly "
    ...
    "commands from memory, and never name or guess what model you are. "
    "Questions about anything else — other bots, "
...
class AI(commands.Cog):
    ...
    def _system_prompt(self, character: str, with_commands: bool = False,
        ...
        if not self._commands.available:
            ...
            parts += [about_block(MODEL_SMART, MODEL_QUICK),
                      self._commands.block(), COMMAND_RULES]
        else:
    ...
    async def _generate(self, guild, channel, user, question, tier, character,
        ...
        attach_cmds = bool(self._commands.available and (
            ...
            or any(w in qlow for w in _MODEL_WORDS)))
